...
function mapCitation(raw: any) {
  const comarNumber = raw.comar_number ?? raw.comarNumber ?? raw.section ?? raw.comar;
  const url = raw.url ?? (comarNumber ? `https://dsd.maryland.gov/regulations/Pages/${String(comarNumber)}.aspx` : undefined);
  return {
    id: raw.id ?? raw.doc_id ?? raw.comar_number ?? randomUUID(),
    label: raw.label ?? (comarNumber ? `COMAR ${String(comarNumber)}` : raw.doc_title ?? "Source"),
    pages: raw.pages ?? "",
    url,
    docId: raw.doc_id,
    docTitle: raw.doc_title ?? raw.title,
    comarNumber: comarNumber ? String(comarNumber) : undefined,
    comarDisplay: raw.comar_display ?? (comarNumber ? `COMAR ${String(comarNumber)}` : undefined),
    snippet: raw.snippet,
  };
}
