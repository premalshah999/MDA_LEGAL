...
const WELCOME: ChatMessage = {
    id: "welcome",
    role: "assistant",
    content: "Ask about Maryland Department of Agriculture/Environment regulations. I'll cite COMAR sources.",
...
export function ChatPanel({ filters, userId, session, loading: sessionLoading, onSessionCreated, onSessionUpdated, onResetSession }: ChatPanelProps) {
  ...
  const send = async () => {
    ...
    setLoading(true);
    // UI engagement: quick status hints
  ...
  const onKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === "Enter") {
      e.preventDefault();
      // If we ever switch to a textarea, Shift+Enter could insert a newline
      // For now (single-line input), always send on Enter
      send();
    }
  };
      ...
      <div className="flex-1 space-y-4 overflow-y-auto p-4 pr-2 max-h-[65vh]">
        ...
        {(sessionLoading ? [
          ...
          <div key={m.id} className="space-y-2">
            ...
            {/* Citations are rendered exclusively in the sidebar SourcesPanel */}
          </div>
        ))}
        <div ref={endRef} />
      </div>
